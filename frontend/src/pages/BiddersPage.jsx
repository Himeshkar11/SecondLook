...
import { getAllBidders, getTenderById } from '../data/demoData.js';

/**
 * BiddersPage — M20
 * Registry of all prospective vendors with compliance status.
 */
export default function BiddersPage() {
  ...
  return (
    ...
    >
      ...
      >
        {[
          { label: 'Verified', count: bidders.filter(b => b.status === 'VERIFIED').length, color: 'var(--color-success)' },
          { label: 'Pending', count: bidders.filter(b => b.status === 'PENDING').length, color: 'var(--color-warning)' },
          { label: 'Flagged', count: bidders.filter(b => b.status === 'FLAGGED').length, color: 'var(--color-danger)' },
          { label: 'Avg. Compliance', count: Math.round(bidders.reduce((s, b) => s + b.compliance, 0) / bidders.length) + '%', color: 'var(--color-primary)' },
        ].map((s) => (
          <div
            key={s.label}
            style={{
          ...
          <table style={{ width: '100%', fontSize: 'var(--font-size-sm)' }}>
            ...
            <tbody>
              {bidders.map((bidder, index) => {
                const tender = getTenderById(bidder.tenderId);
                return (
                  <tr
                    key={bidder.id}
                    style={{
                  ...
                  >
                    ...
                    <td style={{ padding: 'var(--space-3) var(--space-4)', fontSize: 'var(--font-size-xs)', color: 'var(--color-text-secondary)' }}>
                      <button
                        ...
                        onClick={(e) => { e.stopPropagation(); navigate(`/tenders/${encodeURIComponent(bidder.tenderId)}`); }}
                      >
                        {bidder.tenderId}
                      </button>
                    </td>
