...
class DemoEPFOProvider(GovernmentProvider):
    """Deterministic fictional demo provider for EPFO statutory verification (Task 14)."""
    ...
    NOT_FOUND_IDENTIFIER = "EPFO-NOTFOUND-000001"
    UNAVAILABLE_IDENTIFIER = "EPFO-UNAVAIL-000001"

    def __init__(self) -> None:
        self.capabilities = ProviderCapabilities(
            source=self.source,
            identifier_type="EPFO_REGISTRATION",
            supported_fields=[
                "registration_number",
                "establishment_name",
                "status",
    ...
    def verify(self, identifier: str, data: Optional[Dict[str, Any]] = None) -> GovernmentVerificationResponse:
        ...
        if clean_id == self.UNAVAILABLE_IDENTIFIER or "UNAVAIL" in clean_id:
            return GovernmentVerificationResponse(
                ...
                identifier=clean_id,
                message="EPFO verification service is temporarily unavailable.",
                provider="demo",
                is_demo=True,
        ...
        if "ERROR" in clean_id:
            return GovernmentVerificationResponse(
                ...
                identifier=clean_id,
                message="EPFO verification provider returned an error.",
                provider="demo",
                is_demo=True,
                retrieved_at=now_iso,
            )
        ...
        if clean_id in self.DEMO_RECORDS:
            return GovernmentVerificationResponse(
                ...
                message="EPFO registration details found in demo government source.",
                provider="demo",
                is_demo=True,
                retrieved_at=now_iso,
            )
        ...
        return GovernmentVerificationResponse(
            ...
            identifier=clean_id,
            message=f"No EPFO record found for registration number {clean_id}.",
            provider="demo",
            is_demo=True,
            retrieved_at=now_iso,
        )
